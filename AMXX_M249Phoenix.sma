...
#define FIRE_SOUND "weapons/buffm249_shoot1.wav"

#define CSW_BASEDON CSW_M249
#define weapon_basedon "weapon_m249"
new cvar_wpn[8]
#define DAMAGE get_pcvar_float(cvar_wpn[0])
#define CLIP get_pcvar_num(cvar_wpn[1])
#define BPAMMO get_pcvar_num(cvar_wpn[2])
#define SPEED get_pcvar_float(cvar_wpn[3])
#define RECOIL get_pcvar_float(cvar_wpn[4])
#define RELOAD_TIME get_pcvar_float(cvar_wpn[5])
#define SPEED_ZOOM get_pcvar_float(cvar_wpn[6])
#define DAMAGE_ZOOM get_pcvar_float(cvar_wpn[7])
...
public plugin_init()
{
	...
	register_clcmd("get_buffm249", "Get_Weapon")
	cvar_wpn[0]= register_cvar("cvar_buffm249_damage","3.0")
	cvar_wpn[1]= register_cvar("cvar_buffm249_clip_ammo","100")
	cvar_wpn[2]= register_cvar("cvar_buffm249_bp_ammo","200")
	cvar_wpn[3]= register_cvar("cvar_buffm249_speed","1.0")
	cvar_wpn[4]= register_cvar("cvar_buffm249_recoil","0.7")
	cvar_wpn[5]= register_cvar("cvar_buffm249_reload","3.5")
	cvar_wpn[6]= register_cvar("cvar_buffm249_speed_zoom","1.5")
	cvar_wpn[7]= register_cvar("cvar_buffm249_damage_zoom","3.5")
...
public plugin_precache()
{
	...
	precache_sound("weapons/buffm249_scream.wav")
...
{
	register_native("Give_BuffM249", "Get_Weapon", 1)
	register_native("Reset_BuffM249", "Remove_Weapon", 1)	
	register_native("Ammo_BuffM249", "Refill_Weapon", 1)
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_BASEDON && g_Old_Weapon[id] != CSW_BASEDON) && Get_BitVar(g_Had_Weapon, id))
	{
		...
		{
			set_pdata_float(Ent, 46, get_pdata_float(Ent, 46, 4) * SPEED_ZOOM, 4)
		}
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	...
	write_byte(floatround(100.0) / 8) // radius
	write_byte(255) // r
	write_byte(0) // g
	write_byte(0) // b
	write_byte(100) // life <<<<<<<<
...
public fw_TraceAttack_World(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
	...
	if(Get_BitVar(g_Zoom, Attacker)) SetHamParamFloat(3, DAMAGE_ZOOM)
	else SetHamParamFloat(3, DAMAGE)
...
public fw_Weapon_Reload_Post(ent)
{
	...
	PlayEmitSound(id ,"weapons/buffm249_scream.wav", CHAN_ITEM)
	UnSet_BitVar(g_Zoom, id)
	cs_set_user_zoom(id, CS_RESET_ZOOM, 0)
